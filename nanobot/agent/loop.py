...
class AgentLoop:
    ...
    def _persist_no_reply_user_turn(self, session_key: str, msg: InboundMessage) -> None:
        ...
        work_msg = msg
        if msg.media:
            new_content, image_only = reference_non_image_attachments(
        ...
        media_paths = [p for p in (work_msg.media or []) if isinstance(p, str) and p]
        has_text = isinstance(work_msg.content, str) and work_msg.content.strip()
        if has_text or media_paths:
            extra: dict[str, Any] = (
                {"media": list(media_paths)} if media_paths else {}
            )
            extra.update(slack_provenance_from_inbound(work_msg))
            text = work_msg.content if isinstance(work_msg.content, str) else ""
            session.add_message("user", text, **extra)
            self.sessions.save(session)
