...
class AgentRunner:
    ...
    async def _execute_tools(
        ...
        consecutive_tool_call_state: dict[str, Any] | None = None,
        workspace_violation_counts: dict[str, int] | None = None,
        mcp_endpoint_counts: dict[str, int] | None = None,
        mcp_non_retryable_attempts: set[str] | None = None,
        mcp_repeat_blocked_attempts: set[str] | None = None,
        mcp_repeat_blocked_details: dict[str, str] | None = None,
        hook: AgentHook | None = None,
        ...
        if external_lookup_counts is None:
            external_lookup_counts = {}
        if consecutive_tool_call_state is None:
            consecutive_tool_call_state = {}
        if workspace_violation_counts is None:
            workspace_violation_counts = {}
        if mcp_endpoint_counts is None:
            mcp_endpoint_counts = {}
        if mcp_non_retryable_attempts is None:
            mcp_non_retryable_attempts = set()
        if mcp_repeat_blocked_attempts is None:
            mcp_repeat_blocked_attempts = set()
        if mcp_repeat_blocked_details is None:
            mcp_repeat_blocked_details = {}
