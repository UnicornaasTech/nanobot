...
# Instagram draft-review buttons (channels/instagram/); ignore when both channels share one Slack app.
...
class SlackChannel(BaseChannel):
    ...
    async def _on_socket_request(
        ...
        # Fork: skip the reaction when the message is persisted without a reply.
        if not no_reply:
            try:
    ...
    async def _on_block_action(
        ...
        if not actions:
            return
        # Fork: when Instagram and Slack share one Slack app, both Socket Mode
        # clients receive interactive payloads; ignore Instagram draft-review
        # button clicks (action_id starts with "ig_") so they are not forwarded.
