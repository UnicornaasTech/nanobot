...
echo "host.docker.internal -> $HOST_DOCKER_INTERNAL_IP (TCP 2022-2023, 8801-8810, 18000; all other traffic to that address blocked)"
...
iptables -A INPUT -p tcp -m multiport --dports 8900,8765,18790 -j ACCEPT
...
# host.docker.internal: local Whisper STT (2022), macOS TTS MCP (2023), host services (8801-8810), local LLM (18000); block the rest.
iptables -A OUTPUT -p tcp -d "$HOST_DOCKER_INTERNAL_IP" -m multiport --dports 2022,2023,8801:8810,18000 -j ACCEPT
