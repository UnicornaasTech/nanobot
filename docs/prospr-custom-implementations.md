...
- **Empty-response fallback** (`fallback_provider.py`, `runner.py`): When the primary model returns a blank successful reply (after the usual two silent retries on the same model), configured `agents.defaults.fallbackModels` are tried once each before the no-tools finalization prompt — same presets as error failover, skipped when answer text was already streamed. If finalization is still blank, fallbacks are tried once more.
  - **Tool-call fallbacks count as success.** A fallback response with no text but at least one valid `tool_calls` entry is accepted (`FallbackProvider.try_on_empty_response`, default `accept_tool_calls=True`). The runner re-dispatches those tool calls in the same iteration via `_dispatch_tool_calls_phase`, so when the primary emits e.g. a malformed `finish_reason=tool_calls` payload and a fallback model proposes a real `edit_file`/`read_file`, the chain continues instead of dropping into the empty-final-response path.
  - **Finalization fallback uses the same shape as the finalization retry.** `_request_finalization_retry` returns the `retry_messages` it sent, and the runner forwards that list (with the finalization prompt appended) plus `disable_tools=True` and `accept_tool_calls=False` to the fallback request. This prevents the recovery model from being prompted with tools and proposing yet another tool call instead of a final answer.
- **Degenerate `tool_calls` diagnostic** (`openai_compat_provider.py`): `_warn_if_degenerate_tool_calls` logs when a chat-completions response has `finish_reason="tool_calls"` (or `"function_call"`) with an empty `tool_calls` list. Surfaces the known Moonshot-via-OpenRouter shape that triggers the empty-response branch.
