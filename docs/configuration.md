...
## Prospr fork extensions

This repository is a Prospr fork of upstream nanobot. The following config keys are **fork-specific**; full behavior notes live in [`prospr-custom-implementations.md`](./prospr-custom-implementations.md).

| Key | Purpose |
|-----|---------|
| `tools.gmailDraft` | Gmail OAuth credentials for the `create_gmail_draft` tool |
| `agents.defaults.eagerKnowledge` | Promote session traffic into history without waiting for token overflow |
| `agents.defaults.genericMemoryOnly` | Use generic consolidation/archive templates (no channel-specific memory) |
| `channels.instagram` | Instagram DM channel and draft-review workflow |
| `channels.email.dropSubjectPatterns` | IMAP subject filters (fork); outbound email send is disabled by policy |

Upstream keys added in the `d2da6df1` baseline include `tools.file.enable`, `agents.defaults.dream`, transcription providers, WebUI automation settings, and expanded session auto-compact options — see the sections above.
