#!/usr/bin/env python3
"""Fetch a URL with the same guarded ``WebFetchTool`` the agent uses (monkey-patch applied).
...
Guard must be enabled (``patchEnabled`` in ``web-fetch-guard.json`` next to your
``config.json``, or ``NANOBOT_WEB_FETCH_GUARD=1``). Otherwise this prints a warning
and uses the vanilla tool.

Withheld pages match other ``web_fetch`` errors: JSON with ``error``, ``url``, and
``reason`` set to ``content_blocked`` (no page body).
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run nanobot web_fetch after applying the guarded-tool monkey-patch.",
    )
    ...
    from nanobot.contrib.web_guard.bootstrap import apply

    apply()

    from nanobot.agent.tools.web import WebFetchTool

    if not getattr(WebFetchTool, "__nanobot_guarded_web_fetch__", False):
        print(
            "Warning: guarded monkey-patch is not active. "
            "Enable patchEnabled in web-fetch-guard.json (next to config.json) "
            "or set NANOBOT_WEB_FETCH_GUARD=1, then retry.\n"
