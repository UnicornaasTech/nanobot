...
@pytest.mark.asyncio
async def test_send_raises_not_implemented() -> None:
    channel = EmailChannel(_make_config(), MessageBus())
...
@pytest.mark.asyncio
async def test_send_always_raises_even_with_auto_reply_disabled() -> None:
    cfg = _make_config()
...
@pytest.mark.asyncio
async def test_send_raises_for_proactive_recipient() -> None:
    cfg = _make_config()
    cfg.auto_reply_enabled = False
    channel = EmailChannel(cfg, MessageBus())
    with pytest.raises(NotImplementedError, match="create_gmail_draft"):
...
@pytest.mark.asyncio
async def test_send_raises_when_consent_not_granted() -> None:
    cfg = _make_config()
    cfg.consent_granted = False
    channel = EmailChannel(cfg, MessageBus())
    with pytest.raises(NotImplementedError, match="create_gmail_draft"):
...
@pytest.mark.skip(reason="Fork disables SMTP send; use create_gmail_draft")
@pytest.mark.asyncio
