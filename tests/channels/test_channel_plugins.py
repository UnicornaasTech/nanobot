...
def test_optional_dependency_metadata_for_enable():
    ...
    # Fork ships the channel SDKs as core dependencies (unlike upstream,
    # which keeps them as per-channel optional extras).
    for dep_name in (
        "dingtalk-stream",
        "lark-oapi",
        "msgpack",
        ...
        assert any(dep.startswith(dep_name) for dep in required)
    for dependency in (
        "tzdata>=2025.2",
